...
use crate::objects::{CNodeRef, Capability, EpRef, VTableRef};
...
pub fn copy_cap_badged<T: KernelObject>(
    src: &Capability<T>,
    ...
    gsm!()
        .root_cnode()
        .cap_copy_badged(copy_slot.slot(), src.slot.slot(), badge)
        .ok()?;
...
lazy_static! {
    ...
    pub static ref STDIN_CAP: EpRef = EpRef::from_slot_num(ProcessCSpace::Stdin as usize);
    pub static ref STDOUT_CAP: EpRef = EpRef::from_slot_num(ProcessCSpace::Stdout as usize);
    pub static ref STDERR_CAP: EpRef = EpRef::from_slot_num(ProcessCSpace::Stderr as usize);
