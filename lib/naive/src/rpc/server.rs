...
use crate::lmp::{LmpListenerHandle, LmpChannelHandle, LmpMessage};
    ...
    pub async fn run(&mut self) {
        ...
        listener
            ...
            .for_each_concurrent(None, |channel| async {
                ...
                channel.messages().for_each_concurrent(None, |req| async {
...
pub trait RpcRequestHandlers {
    ...
    async fn handle_request_memory(&self, _request: &RequestMemoryRequest) -> Result<(RequestMemoryResponse, Vec<usize>)> {
        Err(Error::CallNotSupported)
    }

    async fn handle_request_irq(&self, _request: &RequestIrqRequest) -> Result<(RequestIrqResponse, Vec<usize>)> {
        Err(Error::CallNotSupported)
    }

    async fn handle_register_service(&self, _request: &RegisterServiceRequest, _cap: Vec<usize>) -> Result<(RegisterServiceResponse, Vec<usize>)> {
        Err(Error::CallNotSupported)
    }

    async fn handle_lookup_service(&self, _request: &LookupServiceRequest) -> Result<(LookupServiceResponse, Vec<usize>)> {
        Err(Error::CallNotSupported)
    }
    ...
    async fn handle_request(&self, channel: LmpChannelHandle, request: LmpMessage) {
        ...
        let (resp_payload, cap) = match opcode {
            ...
            4 => {
                ...
                let (resp, cap)= self.handle_register_service(&request_msg, request.caps).await.unwrap();
                (serde_json::to_vec(&resp).unwrap(), cap)
            }
            5 => {
                let request_msg = serde_json::from_slice(&request.msg).unwrap();
