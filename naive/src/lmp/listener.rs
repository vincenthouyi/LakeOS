...
use crate::{
    space_manager::gsm,
    ep_server::{EpServer, EpMsgHandler},
};
...
impl LmpListener {
    pub fn new(listen_ep: EpCap, badge: usize) -> Self {
        Self { badge: badge, listen_ep: listen_ep }
    }

    pub fn accept_with(&self, c_ntf_ep: EpCap, s_ntf_ep: EpCap, s_ntf_badge: usize) -> Result<LmpChannel, ()> {
        ...
        Ok(LmpChannel::new(c_ntf_ep, s_ntf_ep, s_ntf_badge, argbuf, Role::Server))
    }
}
...
impl EpMsgHandler for LmpListenerHandle {
    fn handle_ipc(&self, ep_server: &EpServer, msg: IpcMessage, cap_transfer_slot: Option<usize>) {
        if let IpcMessage::Message{payload, payload_len, need_reply, cap_transfer, badge} = msg {
...
impl<'a> Stream for IncomingFuture<'a> {
    ...
    {
        Pin::new(&mut self.0.poll_accept()).poll(cx)
            .map(|r| Some(r))
    }
}